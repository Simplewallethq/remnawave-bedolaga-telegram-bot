...
### 🖥️ Автообновление десктопного приложения

Публичный манифест `GET /cabinet/app/update` (версия, ссылка, хеш) настраивается в админке, категория «Обновления приложения». Гайд для разработчика клиента: [docs/app-updates-api.md](docs/app-updates-api.md).
