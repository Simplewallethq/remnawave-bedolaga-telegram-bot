...
class Settings(BaseSettings):
    ...
    # paid_trial: вместо бесплатного триала — «гейт»: символическая оплата
    # TRIAL_PAID_OFFER_PRICE_KOPEKS по СБП (1Payment, с привязкой) даёт
    # TRIAL_PAID_OFFER_ACCESS_DAYS дней тарифа TRIAL_PAID_OFFER_PLAN_CODE;
    # подписке пишется период продления
    ...
    TRIAL_PAID_OFFER_ACCESS_DAYS: int = 3
    ...
    PARTNER_REFERRAL_COMMISSION_PERCENT: int = 60
    ...
    # false — legacy для старых юзеров до TARIFFS_LEGACY_CUTOFF без plan_id.
    #         Тарифы остаются для тех, кто уже на тарифе (plan_id задан), и для всех,
    #         кто зарегался после отсечки.
    ...
    # Кнопка «СБП или картой, разово» на СБП-счёте 1Payment: ведёт на
    # универсальную страницу Platega.
    ...
    def get_trial_paid_offer_access_days(self) -> int:
        return self._int_or_default(self.TRIAL_PAID_OFFER_ACCESS_DAYS, 3, minimum=1)
    ...
    def is_onepayment_card_button_enabled(self) -> bool:
        """Показывать ли на счёте 1Payment (СБП) кнопку разовой оплаты через Platega."""
