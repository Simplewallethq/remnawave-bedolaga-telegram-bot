...
TRIAL_ENTRIES: Tuple[SettingEntry, ...] = (
    ...
    SettingEntry(
        key="TRIAL_PAID_OFFER_FALLBACK_TRIAL_MINUTES",
        section="trial",
        label_ru="🧪 A/B: бесплатный триал неоплатившим через N мин",
        label_en="🧪 A/B: free trial for non-payers after N min",
        action="input",
        description_ru="0 — не выдавать. N>0 — если оффер не оплачен за N минут, выдать обычный триал (проверка раз в минуту).",
        description_en="0 — never. N>0 — grant the regular trial if the offer is unpaid after N minutes (checked every minute).",
    ),
    SettingEntry(
        key="TRIAL_PAID_OFFER_FALLBACK_MAX_AGE_HOURS",
        section="trial",
        label_ru="🧪 A/B: фоллбэк только зарегистрированным за N ч",
        label_en="🧪 A/B: fallback only for users registered within N h",
        action="input",
        description_ru="Кто зарегистрировался раньше, чем N часов назад, фоллбэк-триал не получит. Защита от раздачи всему бэклогу при включении.",
        description_en="Users registered more than N hours ago never get the fallback trial. Guards against mass grants when the setting is switched on.",
    ),
    SettingEntry(
...
def _build_period_options_section(language: str) -> Tuple[str, types.InlineKeyboardMarkup]:
    ...
    subscription_options = (1, 5, 14, 15, 30, 60, 90, 180, 360)
    renewal_options = (1, 5, 15, 30, 60, 90, 180, 360)
